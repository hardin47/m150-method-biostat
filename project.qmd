...
The data can be loaded in directly from Box at: <a href = "https://pomona.box.com/shared/static/knuhli7y7oy3dwhxluanaos5koukc9cy.csv" target = "_blank">https://pomona.box.com/shared/static/knuhli7y7oy3dwhxluanaos5koukc9cy.csv</a>
...
### Due Dates
* **Friday, April 17**. Report whether you will be working in a pair or not.

* **Friday, April 24**. Post (to the GitHub repo that I created) both the Quarto file and the corresponding pdf document:  
...
* (Optional) **Monday, May 4**. If you would like me to look at / provide feedback for a completed rough draft, you should turn in the completed draft before May 4th. (If you are a graduating senior who would like feedback, please provide me a draft by **Friday, May 1st**.)

* **Last week of class or finals week**. Each person will individually talk through their new thing in a 15-20 minute in-person discussion. Notes are welcome!

* **Wednesday, May 6**. Last day of class, each person / group will present their visualization, and there will be secret ballot voting for the best visualization of the data / model.

* **Friday, May 13**.  Complete assignment is due (turn in **both** the qmd and rendered pdf to your GitHub repository). I will be grading from GitHub directly, so no need to turn in to Gradescope. (If you are a graduating senior, the final assignment is due by **noon on Thursday, May 7**.)
...
3. Something new.  Each individual in the class will learn a new idea related to survival analysis (see below).   The new part will be assessed (in person and in writing!!) on your ability to explain the new idea.  That is, you must indicate a complete understanding of the topic as well as an adequate use of the method applied to the dataset. Sometime during the last week or classes or during finals week, you will have an appointment to describe your new idea in person and then you will write about the topic in your report. You are welcome to bring in any notes you might have on the topic.
...
6. The final report which ties it all together should have an introduction, methods, results, discussion.  The <a href = "https://m150-method-biostat.netlify.app/handout/project_rubric.docx" target = "_blank">grading rubric</a> should help you put together the final written project. (Don't forget the reflection, see #5.)
...
* Deriving / detailing AIC & BIC for model selection on Cox PH. Should include a complete understanding of the relevant likelihood.
* Power analysis  (you would either need to explain the details of what the power function does (if you choose to use a built in function) or you need complete a simulation to find the power under various settings of things like sample size and HR)
* Derivation of the sample size calculation for the log rank test (and application to the data)
* An analysis of the Schoenfeld residuals  (**how are they calculated** and why is that calculation relevant to PH?)
* Breslow estimate of the Survival curve under Cox PH assumption.
* Bootstrapping the survival model (what are the assumptions?  what do you conclude? what do you do with the inevitable tied values?)
...
* Lasso or Ridge regression on the Cox PH model (you will need to describe what Lasso / Ridge does and how the coefficients are estimated in the new model paradigm)
...
* If you are working in pairs, the project is extended in three ways.  1.  You must *both* be working on the files in GitHub. That is, each person must make commits to the repository.  2. You must do *two* new items (one each). 3. Each individual must create their own graphic.
...
* Additionally there will be two competitions.  Winning either will add 3 points (out of 100) to your score.
   * Graphic:  the class will vote on who has the best graphic.
   * Model:  using a holdout sample (I only gave you part of the data), I will assess your final model.  The group whose model best describes the holdout sample (as measured by the <a href = "https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4886856/" target = "_blank">c-index</a> with a slight adjustment to benefit models that predict on the highest number of observations) will win the model prize.
