...
(@) I know where to find: the solutions to the warm-ups, the clicker questions (with solutions), and the HW solutions[^37]
     (a) TRUE
     (b) FALSE
...
(@) How many parameters did we estimate in the HERS warm-up with the additive model?[^50]
     a. 1
     b. 3
     c. 4
     d. 2757
...
91. In the log-rank test, why is it okay to consider only one cell of the 2x2 table at time $t_j$?[^91]
...
92. What does it mean for the log rank test to be more powerful than the Wilcoxon test?[^92]
...
93. The hazard at time $t$ represents:[^93]
...
94. The last entry in the table for the h(t) column is `NA` because:[^94]
...
95. Censored observations are$\ldots$?[^95]
...
96. Survival Analysis: for a one unit change of an explanatory variable, the corresponding coefficient $e^\beta$ represents:[^96]
    (a) baseline survival
    (b) survival ratio
    (c) baseline hazard
    (d) hazard ratio
...
97. In survival analysis, the closest interpretation of the value $e^\beta$ is:[^97]
    (a) odds
    (b) probability
    (c) time to event
    (d) relative risk
...
98. Let the event be death.  If larger values of the explanatory variable are associated with higher likelihood of survival, the coefficient $(\beta)$ should be [^98]
    (a) bigger than 1
    (b) smaller than 1
    (c) positive
    (d) negative
...
99. Let the event be death.  If larger values of the variable are NOT associated with higher (or lower) likelihood of survival, the coefficient $(\beta)$ should be[^99]
    (a) bigger than 1
    (b) smaller than 1
    (c) positive
    (d) negative
...
100. BP violates the "linear HR" condition if:[^100]
...
101. A Cox regression analysis:[^101]
...
102. The effect of weight could violate PH if:[^102]
...
103. The effect of treatment could violate PH if:[^103]
...
104. AIC, BIC, model validation, and stepwise regression are methods for[^104]
...
105. If $\alpha = 0.05$, I would expect 5% of all hypotheses to be rejected.[^105]
     (a) TRUE
     (b) FALSE
...
108. For hypothesis testing, the problem of multiple comparisons (also known as the multiple testing problem) results from the increase in ________ that occurs when statistical tests are used repeatedly.[^108]
     (a) Type I errors 
     (b) Type II errors 
     (c) Null hypothesis
...
109. If $H_0$ is true, the p-values should be distributed:[^109]
...
110. Given many many tests (presumably some are null and some are "true"), a good estimate of the number of null tests is:[^110]
     (a) (# p-values > 0.5) / 2
     (b) (# p-values > 0.5) * 2
     (c) (# p-values < 0.5) / 2
     (d) (# p-values < 0.5) * 2
...
111. What do I do if the adjusted p-value is bigger than 1?[^111]
...
112. With Holm’s method, what do I do if the (m+1)^th adjusted p-value is smaller than the m^th adjusted p-value?[^112]
     (a) Leave it unadjusted
...
113. The false discovery rate represents[^113]
...
114. FDR and FWER differ in that[^114]
...
115. Which multiple comparisons adjustment gives the highest power?[^115]
     (a) Bonferonni
     (b) Holm
