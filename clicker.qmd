...
9. The regression technical conditions include:[^8]
...
[^8]: d. The residuals are normally distributed (which induces a., d., and e.).  There is nothing in the technical conditions about the distribution of X (remember, X can be binary!).

---

10. We need the technical conditions to hold in order to calculate $b_0$ and $b_1.$[^9]

    (a) TRUE
    (b) FALSE
    (c) It depends
    
[^9]: FALSE.  We can always minimize the sums of squares, regardless of whether or not the model is any good.

---

11. Why do we check technical conditions?[^10]
...
[^10]: a. so that the inference is valid (and also for fun)

---

12. When writing the regression equation, why is there a hat ( ^) on the response variable?[^11]
...
[^11]: d. due to estimation and average

---

13. With a strong correlation and very small p-value, what can we conclude about happiness and life expectancy?[^12]
...
[^12]: c. happiness and longer life are correlated

---

14. If there is no relationship in the population (true correlation = 0), then r = 0.[^13]
    (a) TRUE
    (b) FALSE

[^13]: b. FALSE, there is no reason that the statistic will equal the parameter.

---

15. If there is no relationship in the population (true slope $\beta_1 = 0$), then $b_1=0$.[^14]
    (a) TRUE
    (b) FALSE
...
16. Smaller variability around the regression line ($\sigma$):[^15]
    (a) increases the variability of $b_1$.
    (b) decreases the variability of $b_1$.
    (c) doesn’t necessarily change the variability of $b_1$.

[^15]: b. decreases the variability of $b_1$.

---

17. Smaller variability in the explanatory variable (SD(X) = $s_X$):[^16]
    (a) increases the variability of $b_1$.
    (b) decreases the variability of $b_1$.
    (c) doesn’t necessarily change the variability of $b_1$.

[^16]: a. increases the variability of $b_1$.

---

18. A smaller sample size ($n$):[^17]
    (a) increases the variability of $b_1$.
    (b) decreases the variability of $b_1$.
    (c) doesn’t necessarily change the variability of $b_1$.

[^17]: b. decreases the variability of $b_1$.

---

19. We transform our variables...[^18]
...
[^18]: d. so that the technical conditions are met.
