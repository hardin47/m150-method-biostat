...
In terms of the prerequisite for Math 150, Methods in Biostatistics, you do not need to know the following topics
...
(@) If the null hypothesis is true, the observed counts will equal the expected counts.[^24b]
     (a) True
     (b) False
...
(@) To reject the null hypothesis we want to see[^24c]
     (a) a small $X^2$ value
     (b) a big $X^2$ value

[^24c]: b. a big $X^2$ value


---

(@) A chi-square test has a[^24d]
...
[^24d]: c. two-sided alt hypothesis, and we only consider the upper end of the sampling distribution
...
(@) Odds Ratio is[^33b]
    (a) the difference of two proportions
    (b) the ratio of two proportions
    (c) the difference of two odds
    (d) the ratio of two odds

[^33b]: d. the ratio of two odds (what are odds? go back to the class notes to make sure you know what odds are!)
...
(@) In finding a CI for RR = p1/p2, why is it okay to exponentiate the ends of the interval for ln(p1/p2)?[^35]
    ...
    (c) Because the natural log compresses values that are bigger than 1 and spreads values that are less than 1.
...
(@) In order to find a CI for the true OR, our steps are:[^36]
 ...
 ii. add $\pm \ z_{1-\alpha/2} \sqrt{\frac{1}{n_1 \hat{p}_1 (1-\hat{p}_1)} + \frac{1}{n_2 \hat{p}_2 (1-\hat{p}_2)}}$
...
(@) Consider the situation where we want to if the probability of success is 0.47  (i.e., $H_0: p = 0.47$):[^48b]
    (a) $L(\hat{p})< L(0.47)$ always
    (b) $L(\hat{p})> L(0.47)$ always
    (c) $L(\hat{p}) \leq L(0.47)$ always
    (d) $L(\hat{p}) \geq L(0.47)$ always

[^48b]: d. $L(\hat{p}) \geq L(0.47)$ always
...
(@) In a logistic regression if $H_0$ is true, what is the probability of success?[^49a]
   (a) $p_0$
...
(@) In algorithmic / computational model building, when selecting variables, it is important that[^61]
...
(@) Which is more important to be high:[^61a]
     a. sensitivity (true positive rate)
     b. specificity (true negative rate)
     c. accuracy

[^61a]: It totally depends on the goals of the model. None of the three metrics can be considered to be more important than the others without knowing the context of the model.

---

(@) You use the training data to create many different models. To get a sense of the true accuracy, the test data should be assessed (for accuracy)[^62]
...
[^65]: b. FALSE.  CV reduces the effect of overfitting, but at the end of the day, you are still building a model on the dataset at hand, and it is possible that you will overfit that dataset.
...
(@) Which variable should I remove first for the backward model process?[^73c]
    (a)	`Location` 
    (b)	`No.eggs`
    (c) `Color`
    (d) `Incubate` 
...
[^73c]: d. `Incubate` has the lowest AIC and biggest p-value.


---

(@) Which variable should I remove second for the backward model process?[^73d]
    (a)	`Location` 
    (b)	`No.eggs`
    (c) `Color`
    (d) `Incubate` 
...
[^73d]: c. `Color` has the smallest test statistic and correspondingly largest p-value.
...
(@) If we remove all the censored observations, the empirical survival curve will (on average)[^86a]
    (a) underestimate the parameter
    (b) overestimate the parameter
    (c) sometimes under and sometimes overestimate the parameter

[^86a]: c. sometimes under and sometimes overestimate the parameter. Because censoring and survival time are independent, there isn't any reason why the censored observations would be different from the other observations. However, removing censored observations isn't ideal because you lose information.
...
(@) Which stopping criteria is most aggressive with respect to stopping early (i.e., is most likely to stop early)?[^116]
     (a) Bonferonni
     (b) Pocock
     (c) Peto
     (d) O'Brien-Fleming
...
(@) Why do we want to stop early?[^117]
     (a) get out positive results sooner
     (b) get out negative results sooner
     (c) use fewer observations (people)
     (d) all of the above
     (e) control the type I error
     
[^117]: d. all of the above. We restrict our analysis to a situation of controlling the type I error. However, that isn't why we **want** to stop early. We **want** to stop early for the other reasons listed.



---

(@) What type of response / outcome variable is Poisson regression designed to model?[^118]
     (a) continuous, normally distributed measurements
     (b) binary yes/no
     (c) non-negative counts
     (d) ordinal ranked categories
     
[^118]: c. non-negative counts

---

(@) What is the canonical link function used in Poisson regression?[^119]
     (a) logit
     (b) identity
     (c) square root
     (d) natural log

[^119]: d. natural log. Remember, the link is the function $g(\cdot)$ such that $g(E[Y|x]) = \beta_0 + \beta_1 \cdot x.$

---

(@) Which of the following would violate one of the Poisson technical conditions?[^120]
     (a) explanatory variables measured on a continuous scale
     (b) response values of 0 for many observations
     (c) the conditional variance being substantially smaller than the conditional mean

[^120]: c. the conditional variance being substantially smaller than the conditional mean
